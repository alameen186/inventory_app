...
<link rel="stylesheet" href="../../assets/css/vendorChat.css">

<div class="chat-shell d-flex flex-column h-100"
     data-init-conv="<cfoutput>#val(url.conversation_id)#</cfoutput>">

    <div class="chat-body-row d-flex flex-grow-1">

        <!-- LEFT: conversation list -->
<div class="chat-left d-flex flex-column border-end bg-light flex-shrink-0"
     style="width:300px; max-width:300px; min-width:220px;">

    <div class="d-flex align-items-center justify-content-between px-3 py-2 bg-white border-bottom flex-shrink-0">
        <h6 class="mb-0 fw-bold">Customer Chats</h6>
        <button id="refreshConvBtn" class="btn btn-outline-secondary btn-sm">&#8635;</button>
    </div>
...
(function(){
    ...
    function loadConversations(cb){
        $.get(CTRL + "?method=getConversations", function(res){
            ...
            $.each(res.data, function(i, c){
                var name    = $('<div>').text(c.other_name || 'Customer').html();
    ...
    function openChat(convId){
        ...
        var name   = nameEl.length ? nameEl.text().trim() : 'Chat';
        $('#chatHeader').text(name).removeClass('text-muted');
   ...
   function fetchMessages(){
    ...
    $.get(CTRL + "?method=getMessages",
        ...
        function(res){
            ...
            if(msgs && msgs.length){
                if(lastMsgId === 0){
                    // Initial load
                    oldestMsgId = res.data.first_msg_id;
                    hasMoreMsgs = res.data.has_more;
                    renderMessages(msgs);
                } else {
                    // Polling — append new messages only
                    appendMessages(msgs);
                }
                lastMsgId = newLastId;
            } else if(lastMsgId === 0){
    ...
    // Save current scroll height before adding content above
    var area          = document.getElementById('messagesArea');
    var scrollBefore  = area.scrollHeight;

    // Show a subtle loading indicator at the top
    $('#olderLoader').show();

    $.get(CTRL + "?method=getMessages",
        { conversation_id: activeConvId, before_id: oldestMsgId },
        function(res){
            $('#olderLoader').hide();
            loadingOlder = false;
            ...
            if(!res.success || !res.data.messages.length){
                ...
                $('#messagesArea').prepend(
                    '<div id="convStart" class="text-center text-muted py-2 small">'
            ...
            var scrollAfter = area.scrollHeight;
    ...
    var loaderHtml = '<div id="olderLoader" class="text-center text-muted py-2 small" style="display:none;">'
        + '<div class="spinner-border spinner-border-sm me-1"></div> Loading older messages...'
        + '</div>';
...
$(document).on('click', '.edit-btn', function(){
    ...
    var $bubble   = $('#bubble-' + msgId);
    /* Get the current plain text (strip the "(edited)" span if present) */
    var $clone    = $bubble.clone();
    $clone.find('span').remove();
    var currentText = $clone.text().trim();

    /* Swap bubble content for an inline editor */
    $bubble.html(
        '<div class="edit-wrap">'
    ...
    $.post(CTRL + "?method=editMessage",
        ...
        function(res){
            if(res.success){
                ...
                /* Restore textarea so user can retry */
...
$(document).on('click', '.cancel-edit-btn', function(){
    ...
    /* origText is already HTML-encoded from data-orig attribute */
